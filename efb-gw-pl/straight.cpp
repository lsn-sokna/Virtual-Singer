...
void diff2(double *x, int xLength, double *ans)
{
...
void interp1Q1(double x, double shift, double *y, int xLength, double *xi, int xiLength, double *ans)
{
	double deltaX;
							   ...
							   double * sliceSTRAIGHT)
{
	...
	{
		preIndex[i]  = min(xLen, max(1, round2((t-t0/4.0)*(double)fs+1+baseIndex[i]) ) ) - 1;
		postIndex[i] = min(xLen, max(1, round2((t+t0/4.0)*(double)fs+1+baseIndex[i]) ) ) - 1;
	}
	...
	for(i = 0;i <= nFragment*2;i++)
	{
		...
		preTime  = (double)baseIndex[i]/(double)fs/(correctionForBlackman/2.0) + 
			((t-t0/4.0)*(double)fs - (double)(round2((t-t0/4.0)*(double)fs))) / (double)fs;
	...
	interp1Q1(dFrequencyAxis, dShift, dSpectrum, fftl*2, xi, ((int)(f0*fftl/(double)fs)*2 + 1), filler);
	...
	interp1Q1(dFrequencyAxis + (double)fs/(double)fftl/2.0, dShift, dSegment, fftl*2, centers, (fftl/2 + 1), lowLevels);
	for(i = 0;i <= fftl/2;i++)
		centers[i] += f0;
	interp1Q1(dFrequencyAxis + (double)fs/(double)fftl/2.0, dShift, dSegment, fftl*2, centers, (fftl/2 + 1), highLevels);
